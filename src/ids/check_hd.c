...
static void threadpool_map(thread_pool_t *th_pool, list_t *hd, void (*routine)(item_t *, item_t *, item_t *, item_t *))
{
  ...
  item_t *item_tmp;
  unsigned mk_calculated_Combine[ITEMS_SPILT_SIZE][ITEMS_SPILT_SIZE] = {0};
  unsigned mk_calculating_group[ITEMS_SPILT_SIZE] = {0};
  struct items_point items_group[ITEMS_SPILT_SIZE] = {0};
  ...
  for (int i = 0; i < ITEMS_SPILT_SIZE; i++) {
  ...
  for(;;) {
    ...
    for (int i = 0; i < ITEMS_SPILT_SIZE; i++) {
      ...
      for (int j = i + 1; j < ITEMS_SPILT_SIZE; j++) {
  ...
  for (int i = 0; i < cur_calculated; i++)
